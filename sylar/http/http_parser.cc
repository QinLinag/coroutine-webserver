...
static sylar::ConfigVar<uint64_t>::ptr g_http_request_max_buffer_size = 
...
static sylar::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =  
        sylar::Config::Lookup("http.request.max_body.size",
            (uint64_t)(64 * 1024 * 1024ull), "http request max body size");

static sylar::ConfigVar<uint64_t>::ptr g_http_response_max_buffer_size = 
...
static sylar::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =  
        sylar::Config::Lookup("http.reponse.max_body.size",
...
uint64_t HttpRequestParser::GetHttpRequestBufferSize() {
    return g_http_request_max_buffer_size->getValue();
}
...
uint64_t HttpResponseParser::GetHttpResponseBufferSize() {
    return g_http_response_max_buffer_size->getValue();
}
...
struct _RequestSizeIniter {
    _RequestSizeIniter() {
        s_http_request_buffer_size = g_http_request_max_buffer_size->getValue();
        s_http_request_max_body_size = g_http_request_max_body_size->getValue();
        s_http_request_buffer_size = g_http_response_max_buffer_size->getValue();
        s_http_request_max_body_size = g_http_response_max_body_size->getValue();

        g_http_request_max_buffer_size->addListener(
        ...
        g_http_response_max_buffer_size->addListener(
                [](const uint64_t& ov, const uint64_t& nv){
...
void on_request_version(void* data, const char* at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        v = 0x11;
    } else (strncmp(at, "HTTP/1.0", length) == 0) {
        v = 0x10;
    } else {
...
//1:成功
//-1：失败
//>0 已经处理的字节数，且data有效数据为len - offset
...
int HttpRequestParser::hasError() {   //成员函数加上const，隐含信息就是this加上const
    return m_error || http_parser_has_error(&m_parser);//我们自己的这个函数是const，但是
}                                                   //http_parser_has_error里面需要修改m_parser


void on_response_reason(void *data, const char *at, size_t length) {
    HttpResponseParser::ptr parser(static_cast<HttpResponseParser*>(data));
