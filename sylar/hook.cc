...
static _HookIniter s_hook_initer;  //这样可以在main函数执行前，就完成初始化，   在main函数执行前，就执行hook_init
...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        ...
        if(rt) {
            SYLAR_LOG_ERROR(g_looger) << hook_fun_name << " addEvent("
                        <<fd << ", " <<event << ")";
            if(timer) {
                timer->cancel();
            }
...
int nanosleep(const struct timespec* req, struct timespec* rem) {
   ...
   sylar::Fiber::ptr fiber = sylar::Fiber::GetThis();
    ...
    iom->addTimer(timeout_ms, std::bind((void(sylar::Scheduler::*)
            (sylar::Fiber::ptr, int thread))&sylar::Scheduler::schedule,
            iom, fiber, -1));
    sylar::Fiber::YieldToHold();
    return 0;
}
...
int socket(int domain, int type, int protocol) {
    ...
    sylar::FdMgr::GetInstance()->get(fd, true);    //当服务端的socket调用bind函数时，就将fd放到了fdmanager中了，
    return fd;
}
...
int accept(int s, struct sockaddr *addr, socklen_t *addrlen) {
    ...
    if(fd >= 0) {
        sylar::FdMgr::GetInstance()->get(fd, true);   //在这里就已经将客户端fd放到了fdmanager里面了，
    }
    return fd;
}
